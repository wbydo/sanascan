...
def insert(session, *, is_develop_mode=True):
    def _insert(morphs):
        insert_stmt = mysql.insert(MorphemeDict)
    ...
    latest_morph_id = session.query(
        MorphemeDict.morpheme_id
    ).order_by(
        MorphemeDict.morpheme_id.desc()
    ).first()
...
def delete(session):
    session.query(MorphemeDict).delete()
...
def _query(session, *, max_req=100):
    columns = Morpheme.__table__.columns.keys()
    for r in ['id', 'sentence_id', 'morpheme_id', 'nth', 'length']:
        columns.remove(r)
